            ...
            {servicesData.map((serv) => (
              ...
              >
                <div className="space-y-5">
                  ...
                  {/* Title */}
                  <h3 className="text-xl font-bold text-slate-900 tracking-tight leading-snug group-hover:text-blue-600 transition-colors font-display">
                    <Link to={`/services/${serv.slug}`} className="focus:outline-none">
                      {serv.title}
                    </Link>
                  </h3>
      ...
      <section className="py-16 bg-white border-t border-slate-100">
        <div className="max-w-4xl mx-auto px-6 text-center space-y-6">
          <h2 className="text-2xl md:text-3.5xl font-extrabold text-slate-900 font-display">
            Engineering High Integrity Portals
          </h2>
          <p className="text-slate-600 text-sm leading-relaxed max-w-2xl mx-auto">
            Our systems integrations are designed around strict security guidelines, WCAG accessibility rules, sitemap crawl indexes, and custom dark branding tokens.
          </p>
          <div className="flex items-center justify-center gap-2 text-xs text-slate-500 bg-slate-50 px-3.5 py-1.5 rounded-full border border-slate-200 inline-block shadow-sm">
            ...
            <span>SLA guaranteed support &amp; monthly reviews</span>
          </div>
        </div>
      </section>
    </div>
