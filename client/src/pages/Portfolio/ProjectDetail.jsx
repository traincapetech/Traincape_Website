...
export default function ProjectDetail() {
  ...
  if (!project) {
    return (
      <div className="min-h-screen bg-[#09090b] flex items-center justify-center text-center p-6">
        <div className="space-y-6 max-w-md">
          <h2 className="text-3xl font-extrabold text-white">Case Study Not Found</h2>
          <p className="text-slate-500 text-sm">
            We couldn't locate the requested enterprise case study portfolio details.
          </p>
          <Link
            to="/portfolio"
            className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white px-5 py-2.5 rounded font-semibold text-sm transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
          >
  ...
  return (
    <div className="bg-[#09090b] min-h-screen text-slate-300 overflow-hidden font-sans">
      ...
      <Helmet>
        <title>{project.seo?.title || `${project.title} Case Study | Traincape Technology`}</title>
        <meta
          name="description"
          content={project.seo?.description || `Explore the technical details and solution architecture of the ${project.title} custom software development project by Traincape Technology.`}
        />
        <meta name="keywords" content={project.seo?.keywords || `${project.title}, software engineering, web application, mobile app`} />
        <link rel="canonical" href={`https://traincapetech.in/portfolio/${project.slug}`} />
        <meta property="og:title" content={project.seo?.title || `${project.title} Case Study`} />
        <meta property="og:description" content={project.seo?.description} />
        <meta property="og:url" content={`https://traincapetech.in/portfolio/${project.slug}`} />
      ...
      <div className="border-b border-slate-800/80 bg-slate-900/10 py-6">
        <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-16 flex items-center justify-between">
          <Link
            to="/portfolio"
            className="inline-flex items-center gap-2 text-slate-400 hover:text-white transition-colors text-sm font-semibold group focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-blue-500 rounded p-1"
          >
      ...
      <article className="max-w-7xl mx-auto px-6 md:px-12 lg:px-16 py-16 lg:py-20 space-y-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <p className="text-slate-300 text-base md:text-lg leading-relaxed border-l-2 border-blue-500/50 pl-4 py-1">
              {project.description}
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 border-t border-b border-slate-800/60 py-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {project.features.map((feat, fidx) => (
                <li
                  key={fidx}
                  className="bg-slate-900/20 border border-slate-800 p-4 rounded-xl flex items-start gap-3 hover:border-slate-700 transition-colors"
                >
          ...
          <div className="lg:col-span-4 bg-slate-900/25 border border-slate-800 rounded-xl p-6 space-y-5">
            <h3 className="font-bold text-sm text-slate-200 uppercase tracking-wider border-b border-slate-800 pb-2.5">
              Technology Stack
            </h3>
        ...
        <div className="space-y-6 pt-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {screenshots.map((shot, sidx) => (
              <div
                key={sidx}
                className="group relative aspect-video bg-slate-950 border border-slate-800 rounded-xl overflow-hidden shadow-2xl"
              >
                <img
                  src={shot.src}
                  ...
                  loading="lazy"
                />
                <div className="absolute inset-0 bg-slate-950/20 group-hover:bg-transparent transition-colors duration-300 pointer-events-none"></div>
                <div className="absolute bottom-3 left-3 bg-[#09090b]/90 border border-slate-800 rounded px-2.5 py-1 text-[10px] text-slate-400 font-semibold uppercase tracking-wider shadow-lg">
                  {shot.type} View
                </div>
              </div>
            ))}
          ...
          <div className="flex flex-wrap items-center justify-center gap-4">
            ...
            {project.website && project.website !== "#" && (
              <a
                ...
                className="flex items-center gap-2 bg-slate-900 border border-slate-800 text-slate-300 hover:text-white px-4 py-2.5 rounded text-xs md:text-sm font-semibold transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-500"
              >
